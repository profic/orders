...
public class OrdersComparableHeap<E extends OrderEntry> {
    ...
    public OrdersComparableHeap(int initialCapacity) {
    ...
    private void siftDown(int index, E el) {
        ...
        while (idx < half) {
            ...
            if (rightIdx < size && child.compareTo((E) queue[rightIdx]) > 0) {
                childIdx = rightIdx;
                child = (E) queue[childIdx];
            }
            if (el.compareTo(child) <= 0) {
                break;
            }
            set(idx, child);
            idx = childIdx;
